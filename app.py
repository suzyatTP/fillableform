...
def submit():
    ...
    # Initialize values
    width, height = letter
    y = height - 90  # y-position to start the content

    # Define rows of content for your table
    rows = [
    ...
    col_width = (width - 100) / 4  # Set column width for your table

    # Loop through rows and draw content on the PDF
    for label, options in rows:
        ...
        p.rect(50, y - row_h, col_width, row_h, stroke=1, fill=0)  # Draw rectangle for the label
        draw_wrapped_text(p, 55, y - 20, label.upper(), col_width - 10, "Helvetica-Bold", 11)  # Draw wrapped label text

        # Draw rectangles and wrapped text for each option
        for i in range(3):
            x = 50 + (i + 1) * col_width
            p.setFont("Helvetica", 10)
            p.rect(x, y - row_h, col_width, row_h, stroke=1, fill=0)  # Draw rectangle for options
            draw_wrapped_text(p, x + 5, y - 20, options[i], col_width - 10)  # Draw wrapped option text
        y -= (row_h + 10)

    # PDF footer content (add your logo, etc.)
    ...
    y = height - 90  # Set starting y-position for the next block of content

    # Add the "Final Decision" and other fields
    ...
    # Save the PDF and prepare to send
    p.save()
    buffer.seek(0)
